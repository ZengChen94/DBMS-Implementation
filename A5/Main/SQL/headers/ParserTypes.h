...
//        Task4
        ...
        for(auto valueToSelect : valuesToSelect) {
            if(!valueToSelect->checkQuery(myCatalog, tablesToProcess)) {
                return;
            }
        }
        for(auto disjunction : allDisjunctions) {
            if(!disjunction->checkQuery(myCatalog, tablesToProcess)) {
                return;
            }
        }
        for(auto groupingClause : groupingClauses) {
            if(!groupingClause->checkQuery(myCatalog, tablesToProcess)) {
                return;
            }
        }
