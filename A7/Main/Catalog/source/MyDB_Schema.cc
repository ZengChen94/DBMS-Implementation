...
void MyDB_Schema::setAtts(string str) {
    auto test = allAtts[0];
    if (test.first.substr(test.first.find("_")+1).find("_") != string::npos){
        resetAtts();
    }
    vector<pair<string, MyDB_AttTypePtr>> tmp;
    for (auto a : allAtts) {
        string s = str + "_" + a.first;
        tmp.push_back(make_pair(s, a.second));
    }
    allAtts = tmp;
}

void MyDB_Schema::resetAtts() {
    vector<pair<string, MyDB_AttTypePtr>> tmp;
    for (auto a : allAtts) {
        int pos = a.first.find("_");
        string s = a.first.substr(pos+1);
        tmp.push_back(make_pair(s, a.second));
    }
    allAtts = tmp;
}
